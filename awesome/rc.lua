...
require("naughty")
-- Vicious library (for dynamic widgets)
require("vicious")
...
globalkeys = awful.util.table.join(globalkeys,
    ...
    awful.key({ modkey, }, "v", function() awful.util.spawn_with_shell('xclip -o -selection clipboard | xclip -i -selection primary; sleep 0.02; xvkbd -no-jump-pointer -xsendevent -text `xsel` 2>/dev/null') end),
...
awful.rules.rules = {
    ...
    -- Set Qutim and gajim to always map on tags number 2 of screen 1.
    { rule = { class = "Qutim" },
...
-- Autostart
awful.util.spawn_with_shell("$HOME/.config/awesome/autostart.sh")
